...
2. Run `make deps` once to install build, image, and virtualization tooling.
3. Run `make image` to compile `bootloader/hello-efi` and stage `/EFI/BOOT/BOOTX64.EFI` inside `images/aios-efi.img`.
4. Run `make run` to launch QEMU with the system disk plus OVMF firmware. You should see "Hello from AIOS – EFI test" in the firmware console; press any key to exit.
